...
Experiment `i` measures duplication on Róża's own JUnit tests before and after implicit-setup refactoring at every clustering level. It loads the current test sources from `src/test/java` at run time, so re-running the experiment always targets the test base as it stands. The experiment is implemented in `src/expt/java/br/ufsc/ine/leb/roza/expt/i/Experiment.java` and uses LCCSS, single linkage, a stable test-order merge tie breaker, and the full agglomerative hierarchy with no extra stop criterion.

The file `experiment-results/i/eligible.csv` contains the baseline metrics for the eligible tests: classes, test methods, setup methods, attributes, total statements, duplicated statements, and duplication rate. The file `experiment-results/i/refactored-by-level.csv` contains the same metrics after refactoring at each clustering level, one row per merge.

## `j`: eligible and per-level refactoring metrics on SAAS tests

Experiment `j` applies the same pipeline as experiment `i` to the SAAS project. It loads tests from `external-projects/saas-unificado/saas+teste` and is implemented in `src/expt/java/br/ufsc/ine/leb/roza/expt/j/Experiment.java`, using LCCSS, single linkage, a stable test-order merge tie breaker, and the full agglomerative hierarchy. Because the SAAS corpus is large, the Gradle task runs with an 8 GB heap.

The file `experiment-results/j/eligible.csv` contains the baseline metrics for the eligible SAAS tests. The file `experiment-results/j/refactored-by-level.csv` contains the same metrics after refactoring at each clustering level.
