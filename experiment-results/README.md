...
Experiment `n` asks whether implicit setup, residual implicit setup, delegated setup, and sequential compositions of those strategies reduce duplicated setup statements across independent open-source test suites, and whether applying a second strategy after the first reduces duplication further than either strategy alone.

**Subject.** The corpus is 22 subjects: 17 open-source libraries under `external-projects/` (each library is one subject even when it has several test modules), Róża's own tests, and four SAAS test modules as separate subjects (`saas+teste`, `saas+teste+moodle`, `saas+teste+selenium`, `saas+teste+service`). The SAAS production and utility trees are excluded.

**Procedure.** Decomposition ignores parser violations so every parseable test enters the pipeline. Implicit and residual-implicit variants decompose with implicit setup, measure similarity with LCCSS, and refactor with the corresponding implicit-setup strategy. Delegated variants decompose without implicit setup, measure similarity with CCS (minimum run length 2), and extract helper methods of that same minimum length. Sequential compositions apply the second strategy to the suite produced by the first. Clustering uses single linkage, a stable test-order tie break, and the full hierarchy; each variant keeps the level with the lowest duplicated setup statements. Wilcoxon signed-rank tests compare each variant with the original suite, Friedman tests compare all eight treatments, Holm-adjusted pairwise Wilcoxon tests compare variants, and composition contrasts test whether the second strategy adds a further reduction. The run writes the three grouped bar charts, a boxplot with one reduction point per project, and a reduction heatmap. `--charts-only` rewrites all five SVGs from the existing `comparison.csv`.

**Findings.** All seven variants reduce duplicated setup statements relative to the original median of 1162. Implicit setup is the weakest gain (median 948.5, −6.5%; 17 improved, 3 worsened, 2 tied; p = 0.028). Residual implicit setup (median 931.5, −11.0%; 20 improved, 0 worsened, 2 tied) and delegated setup (median 905, −11.3%; 19/0/2) are stronger and statistically indistinguishable from each other, while residual implicit setup beats implicit setup (Holm p = 0.003). Compositions improve further: implicit then delegated reaches median 696 (−17.9%), delegated then implicit 703 (−17.4%), residual then delegated 606 (−21.2%), and delegated then residual 693 (−20.4%). Friedman rejects equal treatments (χ² = 102.3, df = 7, p < 0.001). Adding delegated setup after either implicit strategy, or residual implicit setup after delegated setup, reduces duplication beyond the first strategy alone; the two composition orders are not distinguishable. Duplicate reduction coexists with more generated classes and setup methods under implicit strategies (median +185 classes for implicit setup, +13 for residual implicit setup), while delegated setup leaves class and setup counts unchanged and adds helper methods (median +85).
