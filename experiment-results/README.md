...
Experiment `f` evaluates whether applying the selected Róża configuration reduces duplicated statements across 47 undergraduate student programs. The projects are stored in `src/expt/resources/f`, and `src/expt/java/br/ufsc/ine/leb/roza/expt/f/Experiment.java` executes the refactoring with LCCSS, complete linkage, a composed referee that first prioritizes the biggest cluster and then falls back to any cluster, and a similarity-based threshold criterion of 0.4.
...
Experiment `i` measures setup/arrange duplication on Róża's own JUnit tests before and after implicit-setup refactoring at every clustering level. It loads the current test sources from `src/test/java` at run time, so re-running the experiment always targets the test base as it stands. The experiment is implemented in `src/expt/java/br/ufsc/ine/leb/roza/expt/i/Experiment.java` and uses LCCSS measurement, the textual implicit-setup refactorer, single linkage, a stable test-order merge tie breaker, and the full agglomerative hierarchy with no extra stop criterion.

The setup projection contains initialized fields normalized as assignments, statements inside `@Before`/`@BeforeEach` methods, and each accepted test's arrange statements before its first recognized assertion. It excludes uninitialized fields, helper methods, `@After`/`@AfterEach` methods and other fixtures, assertions, and code after the first assertion. Duplicate setup statements use textual surplus counting, `Σ(freq - 1)`, over this same projection before and after refactoring.

The file `experiment-results/i/eligible.csv` contains the filtered original baseline. A class remains in the baseline when it has at least one accepted test, and only methods accepted by decomposition contribute test arrange code. The file `experiment-results/i/controls.csv` separates the decomposed-but-unrecomposed condition from recomposition under the original source-class partition. The file `refactored-by-level.csv` contains the per-level LCCSS curve. `best-level-summary.csv` reports the level with the fewest duplicated setup statements.

## `j`: eligible and per-level refactoring metrics on SAAS tests

Experiment `j` applies the same LCCSS pipeline as experiment `i` to the SAAS project. It loads tests from `external-projects/saas-unificado/saas+teste` and is implemented in `src/expt/java/br/ufsc/ine/leb/roza/expt/j/Experiment.java`, using single linkage, a stable test-order merge tie breaker, and the full agglomerative hierarchy. Because the SAAS corpus is large, the Gradle task runs with an 8 GB heap.

The file `experiment-results/j/eligible.csv` contains the filtered original baseline using the same setup projection as experiment `i`. The file `experiment-results/j/refactored-by-level.csv` uses LCCSS with the textual refactorer.
