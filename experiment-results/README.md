...
**Procedure.** Every parseable test goes through the pipeline. Implicit and residual implicit setup compare tests with LCCSS, while delegated setup uses CCS with a minimum run of two statements. When two strategies run in sequence, the second starts from the suite the first produced. Clustering is single linkage over the full hierarchy, and each variant keeps the level that leaves the fewest duplicated setup statements. Shapiro–Wilk only checks whether the paired differences look normal. Wilcoxon signed-rank tests then compare each variant with the original, Friedman asks whether the eight treatments differ at all, and Holm-adjusted pairwise tests together with planned composition contrasts say which ones pull apart.

**Findings.** Duplicated setup statements fall from a median of 1162. Implicit setup is the weakest of the seven, down to 948.5 (−6.5% per project, p = 0.028). Residual implicit setup and delegated setup both land around −11% and do not differ from each other, but residual implicit setup does beat implicit setup (Holm p = 0.003). Friedman already rejects equal treatments (p < 0.001). Running a second strategy after the first still helps, and residual then delegated is the lowest median, 818.5 (−21%). The cost shows up in structure: implicit setup adds many test classes (median +185), whereas delegated setup keeps the original class layout and instead adds helper methods (median +27).
