...
**Subject.** The corpus is 22 subjects: 17 open-source libraries under `external-projects/` (each library is one subject even when it has several test modules), Róża's own tests, and four SAAS test modules as separate subjects (`saas+teste`, `saas+teste+moodle`, `saas+teste+selenium`, `saas+teste+service`).
