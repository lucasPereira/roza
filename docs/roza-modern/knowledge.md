...
The sixth confirmed pipeline stage is `refactoring`. Its interface is named `TestClassRefactorer`, and its method is named `refactor`. It receives `TestCaseClusters` and returns `RefactoredTestClasses`, which exposes refactored `TestClass` instances through `testClasses()`. It receives the groups produced by `clustering` and decides what to do with them. The first concrete implementation is `ImplicitSetupTestClassRefactorer`, which moves the common initial non-assertion prefix into setup, keeps assertions in test methods, preserves each test method's original annotations and thrown exceptions, and makes generated method names unique inside each generated class. When a moved prefix contains a local array declaration, the generated setup assignment uses explicit array creation because Java does not allow the short array initializer syntax in assignment statements. Setup annotation selection is decided before refactoring: `TestClass` exposes the setup annotation determined by parsing, using the original fixture annotation when present or inferring from `@Test` usage otherwise. `ImplicitSetupTestClassRefactorer` has an implicit-setup package policy: `DEFAULT_PACKAGE` materializes one generated class per cluster in Java's default package to preserve reuse across source packages, while `PACKAGE_PARTITIONING` partitions each cluster by source package and preserves package names in the generated classes. The pipeline may later support other refactoring purposes, such as delegated setup, through different refactoring implementations.
...
- 2026-05-12: Moved modern analytics calculations behind `TestCodeAnalytics` and reduced the UI comparison to classes, tests, setup methods, and fields.
