# Róża Modern Knowledge

This document stores evolving knowledge discovered while designing and implementing modern Róża.

## Vocabulary

- Modern Róża: the modern architecture for Róża, developed under `br.ufsc.ine.leb.roza.core.modern`.
- Legacy Róża: the existing architecture, preserved under `br.ufsc.ine.leb.roza.core.legacy`.
...
- Decomposition stage: the third modern Róża pipeline stage; it separates each test from its original class and carries the code required by that test.
...
- `TestCase`: the model for a decomposed test. It is called a test case because after decomposition it is no longer exactly a method inside a test class. It preserves the original parsed test method name through `name()`, including duplicate names. Its `body()` includes statements derived from supported `@Before` fixtures and preserves assertions. It also preserves original annotations, source context, and thrown exceptions needed by refactoring, but it does not need to preserve the `@Before` annotation itself. It does not expose `id()` yet.
- Measurement stage: the fourth modern Róża pipeline stage; it applies a similarity metric to each pair of decomposed tests and produces a similarity matrix.
...
- `WithoutImplicitSetupTestCaseDecomposer`: a decomposer that keeps only the original test method body in the compared `TestCase`. Fields and `@Before` stay out of measurement. Implicit setup still copies them onto a generated class when the cluster comes from one source class.
- `ContiguousCommonStatementsSimilarityMeasurer` (CCS): Dice of the longest extractable contiguous arrange run shared by two tests. Callers must pass the minimum run length. It is not a substring or subsequence metric. Extractability uses exact normalized statement text, type-compatible live-ins, and at most one live-out.
- `DelegatedSetupTestClassRefactorer`: keeps tests in their original classes and extracts extractable contiguous arrange runs shared by at least two tests in a cluster into static methods on one public helper class per cluster, named `HelperClass1`, `HelperClass2`, and so on.
- `OriginalHelperClassExtractor`: when clustered tests come from source classes that contain helper methods, moves those methods onto one `{OriginalClassName}Helpers` class per original test class in the default package, without rewriting call sites.
...
- Clustering stage: the fifth modern Róża pipeline stage; it groups similar tests using only the similarity matrix.
...
- Writing stage: the final modern Róża pipeline stage; it writes refactored test classes to an output destination such as the filesystem or cloud.
...
- Modern UI: the JavaFX manual-testing UI for modern Róża starts under `br.ufsc.ine.leb.roza.ui.modern` and starts from `ModernRozaUi`.

## Current Architectural Understanding

Modern Róża is intended to be a reusable and extensible framework for automated test-code refactoring. Its central abstraction is a pipeline composed of independent stages. Each stage should be customizable through interchangeable implementations.
...
Modern and legacy Róża are isolated architecture areas. The isolation is bidirectional: `core.modern` must not import `core.legacy`, and `core.legacy` must not import `core.modern`.
...
The third confirmed pipeline stage is `decomposition`. Its interface is named `TestCaseDecomposer`, and its method is named `decompose`. It receives parsed test classes and separates each supported test from its class of origin. `TestCaseDecomposer.decompose` returns `DecomposedTestCases`, which exposes decomposed `TestCase` instances through `testCases()`. Each decomposed test is represented by `TestCase`. The name distinguishes decomposed tests from parsed `TestMethod` models, which still represent methods inside a `TestClass`. `TestCase` preserves the original parsed test method name for later refactoring use, even when names are duplicated. It also preserves the source `TestClass` reference, original parsed test method annotations, and thrown exceptions from supported source fixture/test methods so refactoring can reuse imports, setup annotation decisions, method-level JUnit style, and required `throws` clauses. Assertions remain in the full `TestCase` body; measurement-specific projections decide whether to stop before assertions. For now, `TestCase` does not expose `id()`. In the implicit setup example, a class with one implicit setup and two tests produces two decomposed test case models, each containing field declarations, the implicit setup statements, and the body of one test. When a supported `@Before` statement assigns to a parsed field, decomposition replaces that assignment with an initialized local declaration such as `TestCase alpha = new TestCase(...)`; this avoids producing a declaration-only prefix followed by a separate assignment. Decomposition skips classes and test methods listed in `ParsedTestClasses.violations()`, so measurement and refactoring operate only on the supported subset by construction.

The fourth confirmed pipeline stage is `measurement`. Its interface is named `TestCaseSimilarityMeasurer`, and its method is named `measure`. It receives `DecomposedTestCases` and returns `TestCaseSimilarityMatrix`. It applies a similarity metric to each pair of decomposed test cases. Similarity metrics can have different objectives, with duplicated test/code identification as the most common one. The returned matrix records the similarity degree for each directed pair according to the applied metric. The first matrix implementation receives the ordered `TestCase` list, stores scores in a dense `double[]`, initializes all values as `0.0`, initializes the diagonal as `1.0`, exposes package-level `setSimilarity(int sourceIndex, int targetIndex, double similarity)` for measurers, and exposes public read methods for `size()`, `testCaseAt(int)`, and `similarity(int, int)`. Measurement may use a projection of the `TestCase` body instead of every statement. LCCSS, GAP, MAP, SEP, LCS, Deckard, JPlag, and Simian use the pre-assertion projection, stopping at the first assertion statement using `CodeStatement.isAssertion()` rather than inferring assertions from statement text. LCCSS counts the common contiguous prefix from the start of both projected statement lists, and its score is `(2 * commonPrefixSize) / (sourceProjectionSize + targetProjectionSize)`. GAP greedily matches dependency-ready statements after considering each test as the textual reference. MAP searches the corresponding choices up to 50,000 nodes and falls back to GAP if that limit is exceeded. Both select the larger prefix found with either reference, normalize it with the same Dice formula, and store a symmetric score. Their dependency graph preserves read-after-write, write-after-read, and write-after-write ordering; parse failure falls back to textual prefix matching. LCS counts the longest common subsequence over the projected statements while preserving order, and its score is `(2 * commonSubsequenceSize) / (sourceProjectionSize + targetProjectionSize)`. JPlag uses the directional percentages reported in each `match*-top.html` file as directed scores. Deckard and Simian use asymmetric source line coverage over the projected test body, merging overlapping intervals and excluding wrapper lines introduced only for tool execution.
...
The sixth confirmed pipeline stage is `refactoring`. Its interface is named `TestClassRefactorer`, and its method is named `refactor`. It receives `TestCaseClusters` and returns `RefactoredTestClasses`, which exposes refactored `TestClass` instances through `testClasses()`. It receives the groups produced by `clustering` and decides what to do with them. The first concrete implementation is `ImplicitSetupTestClassRefactorer`, which moves the common initial non-assertion prefix into setup, keeps assertions in test methods, preserves each test method's original annotations and thrown exceptions, and makes generated method names unique inside each generated class. When a moved prefix contains a local array declaration, the generated setup assignment uses explicit array creation because Java does not allow the short array initializer syntax in assignment statements. Setup annotation selection is decided before refactoring: `TestClass` exposes the setup annotation determined by parsing, using the original fixture annotation when present or inferring from `@Test` usage otherwise. That first refactorer materializes one generated class per cluster in Java's default package, including singleton clusters whose decomposed bodies lose the original class-level fixture. `ResidualImplicitSetupTestClassRefactorer` is a second implicit-setup implementation: clusters with two or more tests still receive that extraction, while singleton leftovers are regrouped by original class and emitted as residual classes that keep the original name, package, fields, fixtures, and original test method bodies. Helper methods that belonged to those source classes are moved to `{OriginalClassName}Helpers` classes instead of remaining on residual or generated test classes. Residual classes therefore preserve general-fixture reuse that the default implicit-setup strategy would inline into one-test classes. The pipeline may later support other refactoring purposes, such as delegated setup, through different refactoring implementations.

The final confirmed pipeline stage is `writing`. Its interface is named `TestClassWriter`, and its method is named `write`. It receives `RefactoredTestClasses` and writes them to an output destination without returning a serialized model. The destination may be the filesystem or a cloud target, depending on the writer implementation. Concrete writer implementations receive required output destination parameters through constructors. The first concrete implementation is `FileSystemTestClassWriter`, which uses `JunitTestClassRenderer` to write one `.java` file per refactored `TestClass` into a configured output folder. Test classes and created helper classes are written in Java's default package at the folder root. Existing helper classes keep their original packages and are written under the matching directory path.

The first modern UI slice uses JavaFX 17.x while the project remains on Java 11. It is a code-only skeleton without FXML. Its layout mirrors the pipeline: a top stage bar, a left sidebar with selected-stage configuration and action button, and a center area with placeholder data produced by the previous stage. The layout should stay minimal, using mostly black, white, and gray; pipeline status colors are exceptions. The top stage bar and selected-stage action button use `#333333`, and blocked pipeline stage buttons use a lighter gray treatment without JavaFX disabled opacity so their text remains readable. Stage selection is visual state independent from pipeline status, so a previous completed stage can still be shown as selected. The loading stage is the first functional UI flow: it defaults the source folder to Róża's own `src/test/java`, opens a folder selector, lets the user choose recursive loading and accepted `.java`/`.txt` extensions, calls `FileSystemCodeFileLoader`, and shows the loaded files in the `Parsing` center. Selecting a loaded file shows its content. The parsing configuration only exposes parser implementation selection, not unsupported feature policy selection. The `Parsing` center shows parsing violations above the loaded-file viewer when violations exist, one violation at a time, with previous/next controls; showing a violation selects the loaded file that contains the referenced test class, uses `ClassName.methodName` format for method violations, and shows the violation code snippet on the normal background without a card-style white wrapper. The `Decomposition` center summarizes classes with class-level violations, tests with method-level violations, tests excluded by violations, accepted tests, and helper files. After `Test classes`, a `Helper classes` tab shows helper-class source code only, and `Violations` is ordered by description. The `Measurement` center shows decomposed tests from the previous stage and the code for the selected test, with a metric dropdown that defaults to LCCSS and also includes LCS, Deckard, JPlag, and Simian. Triggering `Measure` produces the selected metric's similarity matrix. The `Clustering` center owns the source/target selectors, shows a narrow ranked source-target similarity score list with an order toggle, omits self-pairs, preserves ranking scroll when selecting an item, synchronizes both source and target controls from ranked-list selection, and shows side-by-side source and target decomposed-code blocks. Triggering `Refactor` runs the selected refactoring strategy over the clustering output. The `Writing` center shows inner tabs `Test classes` and `Helpers classes`, each with a class list and rendered Java code on the right. After `Writing` completes, the top toolbar shows an `Analitycs` tab in purple; rerunning an earlier stage hides it again. The `Analitycs` center shows side-by-side tables for original-code violation statistics and an original-vs-refactored metric comparison. Original test-class and test-method counts in the comparison reuse the same "without violations" baseline shown in the original-code table.
...
- `ResidualImplicitSetupTestClassRefactorer` is implemented in `src/main/java/br/ufsc/ine/leb/roza/core/modern/refactoring` and covered by tests in `src/test/java/br/ufsc/ine/leb/roza/core/modern/refactoring`.
...
- **Modern UI spacing (`ModernRozaUi`)**: Use two layout gaps only. **`SPACING_X` (8px)** separates controls within the same group (title + field, combo + value + remove, cluster tile internals). **`SPACING_4X` (32px)** separates distinct groups or major sections (configuration blocks, sidebar children, content columns, action button after configuration). Nest `VBox`/`HBox` groups instead of ad hoc `VBox.setMargin` tuning. Stages without visible configuration controls add the action button directly to the sidebar so their first visible control aligns with loading and refactoring. **`ComboBox` height**: force a single-line control with explicit **`-fx-min-height` / `-fx-pref-height` / `-fx-max-height`** (30px in the first implementation) and modest horizontal padding.
- **Modern UI writing output (`ModernRozaUi`)**: The `Writing` sidebar follows the loading folder-selector pattern for the output destination and defaults to `output/writer` when the app runs from the Róża project.
...
- DEC-001: Existing Róża code was moved to `core.legacy`; new architecture work belongs in `core.modern`.
- DEC-002: Durable requirements for modern Róża are stored in `docs/roza-modern/requirements.md`.
- DEC-003: Evolving model knowledge and implementation discoveries are stored in this file.
- DEC-004: Agents working on modern Róża should use the `roza-modern-development` skill.
- DEC-005: Automated test-code refactoring pipelines are not unique to modern Róża; modern Róża work must be distinguished by explicit references to modern Róża, `core.modern`, or the modern architecture documents.
...
- DEC-007: Modern Róża is currently understood as loading raw code files as initial input, not as being defined by receiving a whole code project as input.
- DEC-008: The first confirmed pipeline stage is `loading`; its interface is named `CodeFileLoader`.
- DEC-009: The loading interface's `load` method loads raw code files, not parsed modern Róża test models.
...
- DEC-013: Modern Róża must be implemented from scratch and must not use or reuse classes from legacy Róża.
...
- DEC-016: Detailed `TestClass` design is deferred until the whole modern Róża pipeline is understood at a high level.
...
- DEC-022: The first concrete purpose of modern Róża is regrouping tests into better classes to enable implicit setup, while broader refactoring generality remains a desired but still uncertain design goal.
...
- DEC-036: Modern Róża code should not include comments unless they explain non-obvious information.
...
- DEC-041: JavaParser is allowed inside concrete Java parser implementations, but JavaParser types must not appear in the public modern Róża parsing contract.
...
- DEC-067: The modern JavaFX UI uses `SPACING_X` (8px) for intragroup gaps and `SPACING_4X` (32px) for extragroup gaps, nested `VBox`/`HBox` groups instead of margin-based tuning, direct action-button placement for stages without visible configuration controls, and explicit single-line `ComboBox` height.
...
- DEC-070: Assertion detection is a syntactic operational model: Róża stops metrics at detectable assertions, not at every possible semantic assertion hidden behind arbitrary calls.
...
- DEC-091: Modern JPlag integration is gated by actual JPlag execution. On this machine, the existing `jplag-2.11.9.jar` executes successfully through the script-style arguments used by legacy Róża.
...
- DEC-094: Modern Simian integration is gated by actual Simian execution. On this machine, the existing `simian-2.5.10.jar` executes successfully through the script-style arguments used by legacy Róża.
...
- DEC-113: The modern UI `Writing` tab shows inner tabs `Test classes` and `Helpers classes`, each with a list and rendered code for the selected class. Both lists use natural name order so numbered suffixes follow numeric order.
...
- DEC-133: Before/after duplication comparisons use `SetupCodeProjection`: initialized fields normalized as assignments, `BEFORE` fixture statements, accepted test statements before the first assertion, and helper-method bodies from helper classes (assertions and `return` statements omitted). Uninitialized fields, later fixtures, assertions, and post-assertion code are excluded. Helper calls that remain in test arrange are counted; helper bodies are counted once.
- DEC-134: Setup duplication is textual surplus `Σ(freq - 1)` over the same `SetupCodeProjection` before and after refactoring.
- DEC-135: The eligible original baseline preserves each accepted source class's fields and fixtures once and includes exactly the test methods accepted by `TestCodeEligibility`; a method-level violation does not exclude other valid methods in that class.
- DEC-136: Setup Extraction Potential (SEP) uses the same common pre-assertion prefix as LCCSS and normalizes it as `commonPrefixSize / maximumMethodSize` so scores stay in `[0.0, 1.0]` while longer prefixes still outrank shorter ones. If the size is omitted, SEP uses the largest arrange in the measured tests.
- DEC-137: GAP and MAP evaluate both tests as possible textual references, select the larger dependency-respecting prefix, apply `DiceMatchSimilarity`, and store the result symmetrically.
- DEC-138: Arrange dependency analysis preserves read-after-write, write-after-read, and write-after-write ordering over JavaParser-derived definitions and uses; parse failure falls back to textual prefix matching.
- DEC-139: MAP bounds its search at 50,000 visited states by default and falls back to GAP when the limit is exceeded.
...
- DEC-141: `ResidualImplicitSetupTestClassRefactorer` extracts implicit setup only for clusters with two or more tests; singleton leftovers return to residual original classes using the original test methods rather than decomposed bodies.
- DEC-142: The modern UI `Refactoring` configuration selects the refactoring strategy; implicit setup remains the default, and both Refactor actions use the selected strategy.
...
- DEC-144: `WithoutImplicitSetupTestCaseDecomposer` is the decomposer for delegated setup and for comparing tests without implicit setup. It copies each original test method body and does not inline fields or `@Before` statements. Implicit setup still materializes those source fields and `@Before` on the generated class when every clustered test comes from that class.
...
- DEC-158: Delegated setup extracts a contiguous arrange run when at least two tests in the cluster share it. The extraction does not require every test in the cluster to contain the run. A test with empty arrange no longer suppresses helpers for the other tests.
...
- DEC-160: Delegated setup caches n-way extraction by cluster indexes so Top ranking does not re-extract the same cluster at every dendrogram level. `ExtractableArrangeRuns` parses each test body once per extraction. The ranking worker always clears its computing state. `nWay` selects runs one arrange length at a time, longest first, so it does not keep every window of every length in memory together.
...
- 2026-05-10: Created initial knowledge base for modern Róża.
- 2026-05-10: Clarified that generic Róża pipeline/refactoring work can also describe legacy Róża and should not automatically trigger modern Róża requirements.
- 2026-05-10: Clarified the separation between user-confirmed requirements and agent-maintained understanding.
- 2026-05-10: Updated understanding to focus modern Róża input on loaded tests.
...
- 2026-05-10: Recorded that modern Róża must not use or reuse legacy Róża classes.
...
- 2026-05-11: Split UI packages into `ui.legacy` and `ui.modern` to prepare for a new modern Róża UI.
...
- 2026-05-11: Recorded the default modern UI loading source folder as Róża's own test source directory.
...
- 2026-08-21: Removed implicit-setup package policy configuration; `ImplicitSetupTestClassRefactorer` always materializes generated classes in Java's default package.
...
- 2026-05-12: Added the modern UI `Analitycs` tab as a post-writing view with original/refactored statistics and recorded its visibility/reset behavior.
- 2026-08-22: Unified setup-only before/after measurement, corrected method-level eligibility, and added experiment `i` controls and best-level summaries.
- 2026-08-22: Removed the experimental GAP/MAP metrics and admissible refactorer from the modern pipeline, UI, and experiments to focus on LCCSS with textual implicit setup.
- 2026-08-22: Restored GAP and MAP as bounded dependency-aware measurement alternatives, without restoring the admissible refactorer or adding experiment runs.
- 2026-08-23: Implemented `ResidualImplicitSetupTestClassRefactorer` so singleton leftovers return to residual original classes (DEC-141).
- 2026-08-23: Added a refactoring-strategy dropdown to the modern UI `Refactoring` configuration (DEC-142).
- 2026-08-23: Implemented delegated setup: CCS measurement, a body-only decomposer, helper-class extraction, and helper-body counting in `SetupCodeProjection` (DEC-143 to DEC-150).
- 2026-08-23: `FileSystemTestClassWriter` empties the configured output folder before writing (DEC-151).
- 2026-08-23: Helper classes are detected by the parser from the single source folder, written with existing packages or the `HelperClass` prefix in the default package, and shown in dedicated Decomposition and Writing tabs (DEC-147 to DEC-152).
- 2026-08-23: The modern UI `Measurement` test list scrolls back to the selected test when the tab is rebuilt (DEC-153).
- 2026-08-23: SEP now divides the shared prefix by a maximum method size so scores stay between 0 and 1 (DEC-136).
- 2026-08-23: The modern UI `Decomposition` tabs are `Test classes`, `Helper classes`, and `Violations` ordered by description (DEC-152).
- 2026-08-23: The modern UI `Writing` `Helpers classes` tab lists helpers ordered by name (DEC-113).
- 2026-08-23: Writing class lists use natural name order so numbered suffixes follow numeric order (DEC-113).
- 2026-08-23: Parsing, Decomposition, and Measurement lists use the same natural name order (DEC-154).
...
- 2026-08-24: Implicit setup restores source fields and `@Before` when the decomposer omitted them from measurement (DEC-144).
...
- 2026-08-28: CCS requires an explicit minimum run length. Experiment n uses 2.
- 2026-08-30: `ExtractableArrangeRuns.nWay` selects by decreasing window length so peak memory stays at one length. Experiment n passes minimum length 2 to `DelegatedSetupTestClassRefactorer`, matching CCS.
