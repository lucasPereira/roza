...
- `SetupCodeProjection`: the common before/after analytics projection containing initialized fields as assignments, `BEFORE` fixture statements, and each accepted test's statements before its first assertion.
...
- DEC-086: The modern UI `Measurement` metric dropdown offers LCCSS by default and GAP, MAP, SEP, LCS, Deckard, JPlag, and Simian as alternatives.
...
- DEC-125: `DiceMatchSimilarity` is the shared Dice normalization `(2 × m) / (|α| + |β|)` used by LCCSS, GAP, MAP, and LCS; it is not itself a metric name.
...
- DEC-133: Before/after duplication comparisons use `SetupCodeProjection`: initialized fields normalized as assignments, `BEFORE` fixture statements, and accepted test statements before the first assertion. Uninitialized fields, helpers, later fixtures, assertions, and post-assertion code are excluded.
...
- DEC-136: Setup Extraction Potential (SEP) is a separate measurement metric from LCCSS; it reports the absolute common pre-assertion prefix length so longer extractable setups outrank equally proportional shorter setups without changing LCCSS normalization.
