package br.ufsc.ine.leb.roza.expt.f;
...
import br.ufsc.ine.leb.roza.core.utils.ShapiroWilkNormalityTest;
import br.ufsc.ine.leb.roza.core.utils.ShapiroWilkNormalityTest.NormalityTestResult;
import br.ufsc.ine.leb.roza.core.utils.WilcoxonSignedRankTest;
import br.ufsc.ine.leb.roza.core.utils.WilcoxonSignedRankTest.WilcoxonTestResult;
...
public class Experiment {
	public static void main(String[] args) {
		...
		FolderUtils folderUtils = new FolderUtils("experiment-results/f");
		...
		for (int project = 1; project <= 47; project++) {
			try {
				new FolderUtils("output/materializer").createEmptyFolder();

				TextFileLoader loader = new RecursiveTextFileLoader("src/expt/resources/f/p" + project + "/original");
				...
				new FolderUtils("src/expt/resources/f/p" + project + "/refactored").createEmptyFolder();
				TestClassWriter writer = new Junit4TestClassWriter("src/expt/resources/f/p" + project + "/refactored");
				writer.write(refactoredClassses);

				TextFileLoader refactoredLoader = new RecursiveTextFileLoader("src/expt/resources/f/p" + project + "/refactored");
				...
				System.out.println("==================");
				System.out.printf("Project %d failed%n", project);
				throw exception;
			}
		}
		...
		List<Integer> uniqueDifferences = new ArrayList<>();
		for (int index = 0; index < originalUniqueDuplicatesList.size(); index++) {
			uniqueDifferences.add(originalUniqueDuplicatesList.get(index) - refactoredUniqueDuplicatesList.get(index));
		}

		List<Integer> totalDifferences = new ArrayList<>();
		for (int index = 0; index < originalTotalDuplicatesList.size(); index++) {
			totalDifferences.add(originalTotalDuplicatesList.get(index) - refactoredTotalDuplicatesList.get(index));
		}

		ShapiroWilkNormalityTest normalityTest = new ShapiroWilkNormalityTest();
		NormalityTestResult uniqueDifferencesNormality = normalityTest.test(uniqueDifferences);
		NormalityTestResult totalDifferencesNormality = normalityTest.test(totalDifferences);

		System.out.println("==================");
		System.out.println("Normality Test (Shapiro-Wilk)");
		System.out.println("------------------");
		System.out.printf("Unique duplicates (original - refactored) - %s%n", uniqueDifferencesNormality);
		System.out.printf("Total duplicates (original - refactored) - %s%n", totalDifferencesNormality);
