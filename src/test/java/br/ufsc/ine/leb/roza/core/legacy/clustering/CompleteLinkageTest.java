...
import br.ufsc.ine.leb.roza.core.legacy.SimilarityReportBuilder;
...
class CompleteLinkageTest {
	...
	void singleAndNonSingleElementClustersWithSymmetricSimilarity() {
		...
		Linkage linkage = new CompleteLinkage(builder.build());
		assertEquals(dotTwo, linkage.evaluate(alphaBetaCluster, gammaCluster).stripTrailingZeros());
	}
	...
	void nonSingleElementClustersWithSymmetricSimilarity() {
		...
		Linkage linkage = new CompleteLinkage(builder.build());
		assertEquals(dotTwo, linkage.evaluate(alphaBetaCluster, gammaDeltaCluster).stripTrailingZeros());
	}

	@Test
	void singleElementClustersWithAsymmetricSymilarityWithTheSameValues() {
	...
	@Test
	void singleElementClustersWithAsymmetricSymilarityWithTheDistinctValues() {
		SimilarityReportBuilder builder = new SimilarityReportBuilder(false);
		builder.add(alpha, beta, dotTwo);
		builder.add(beta, alpha, dotEight);
		Linkage linkage = new CompleteLinkage(builder.build());
		assertEquals(dotTwo, linkage.evaluate(alphaCluster, betaCluster));
	}
